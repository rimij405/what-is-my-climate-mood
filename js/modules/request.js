...
export let request = (function() {
    ...
    const configns = (function(){    
        ...
        function RequestConfiguration(mimeType, action, uri, bustCache, callbacks){
        ...
        RequestConfiguration.prototype.bustCache = function(value){
            this.cacheBuster = (value === true) ? value : false;
        }
        ...
        function CreateRequestConfiguration(mimeType, action, uri, cacheBust, callback){
            return new RequestConfiguration(mimeType, action, uri, cacheBust, callback);
        }
    ...
    const requestns = (function(){
        ...
        function Request(config){
            this.options = config ? config : configns.RequestConfiguration(
                "application/json",
                "GET",
                "config.json"
            );
        ...
        Request.prototype.open = function(body){
            ...
            // Open the request.
            let address = `${this.options.url}${this.options.cacheBuster ? `?${new Date().getTime()}` : ''}`;
